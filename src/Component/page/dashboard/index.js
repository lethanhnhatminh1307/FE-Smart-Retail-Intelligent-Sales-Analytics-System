...
function Dashboard() {
    ...
    const filteredProducts = products.filter((p) => p.name && p.name.toLowerCase().includes(searchText.toLowerCase()));
                ...
                <Row gutter={[24, 24]}>
                    <Col xs={24} sm={12} lg={8}>
                        <Card className={cx('statCard', 'inventory')}>
                            <div className={cx('title')}>Doanh thu hôm nay</div>
                            <div className={cx('value')}>{formatCurrency(summary?.day?.totalRevenue || 0)}</div>
                            <div style={{ marginTop: 12, color: '#d4380d' }}>
                                <InboxOutlined style={{ marginRight: 8 }} />
                                Số hàng bán ra: {summary?.day?.totalOrders || 0}
                            </div>
                        </Card>
                    </Col>
                    <Col xs={24} sm={12} lg={8}>
                        <Card className={cx('statCard', 'revenue')}>
                            <div className={cx('title')}>Doanh thu tuần này</div>
                            <div className={cx('value')}>{formatCurrency(summary?.week?.totalRevenue || 0)}</div>
                            <div style={{ marginTop: 12, color: '#389e0d' }}>
                                <InboxOutlined style={{ marginRight: 8 }} />
                                Số hàng bán ra: {summary?.week?.totalOrders || 0}
                            </div>
                        </Card>
                    </Col>
                    <Col xs={24} sm={12} lg={8}>
                        <Card className={cx('statCard', 'profit')}>
                            <div className={cx('title')}>Doanh thu tháng này</div>
                            <div className={cx('value')}>{formatCurrency(summary?.month?.totalRevenue || 0)}</div>
                            <div style={{ marginTop: 12, color: '#096dd9' }}>
                                <InboxOutlined style={{ marginRight: 8 }} />
                                Số hàng bán ra: {summary?.month?.totalOrders || 0}
                            </div>
                        </Card>
                    </Col>
                </Row>
                ...
                <Row gutter={[24, 24]}>
                    ...
                    <Col xs={24} sm={12} lg={6}>
                        <Card className={cx('statCard', 'revenue')}>
                            ...
                            <div className={cx('value')}>{formatCurrency(summary.actualRevenue)}</div>
                            <div style={{ marginTop: 12, color: '#389e0d' }}>
                    ...
                    <Col xs={24} sm={12} lg={6}>
                        <Card className={cx('statCard', 'profit')}>
                            ...
                            <div className={cx('value')}>{formatCurrency(summary.actualProfit)}</div>
                            <div style={{ marginTop: 12, color: '#096dd9' }}>
